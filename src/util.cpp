...
int nAnonymizeXCYPTAmount = 50000;
...
#else
    const char* pszModule = "cryptcash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CryptCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CryptCash
    // Mac: ~/Library/Application Support/CryptCash
    // Unix: ~/.cryptcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CryptCash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cryptcash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cryptcashd.pid"));
