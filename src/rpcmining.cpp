...
// This is a NON FUNCTIONAL RPC FUNCTION! PLEASE DO NOT UNCOMMMENT THIS
	/*
		uint64_t nCoinAge;
		CTxDB txdb("r");
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "CryptCash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "CryptCash is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CryptCash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CryptCash is downloading blocks...");
