...
const string strMessageMagic = "CryptCash Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int64_t nHeight, bool fMasternodePayment)
{
	int64_t nSubsidy = 0 * COIN;
	
	if (nHeight < 10000)
	{
		nSubsidy = nCoinAge * (1 + .5 / 100.) * 33 / (365 * 33 + 8) + fMasternodePayment * Params().MasternodeFixReward();
    }
    else nSubsidy = .5 * COIN + fMasternodePayment * Params().MasternodeFixReward();
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees, pindex->nHeight, HasMasternodePayment());
...
{
    RenameThread("cryptcash-loadblk");
