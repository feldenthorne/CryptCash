...
void ThreadIRCSeed()
{
    ...
    RenameThread("cryptcash-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #xscp9348nTEST\r");
            Send(hSocket, "WHO #xdvp9348n\r");
        } else
        {
        LogPrintf("JOIN #xscp9348n\n");
            // randomly join #cryptcashcoin00-#cryptcashcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #xscp9348n%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #xscp9348n%02d\r", channel_number).c_str());
        };
