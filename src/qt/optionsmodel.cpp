...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXCYPTAmount"))
        settings.setValue("nAnonymizeXCYPTAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeXCYPTAmount = settings.value("nAnonymizeXCYPTAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeXCYPTAmount"))
        SoftSetArg("-anonymizeXCYPTamount", settings.value("nAnonymizeXCYPTAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case anonymizeXCYPTAmount:
            nAnonymizeXCYPTAmount = value.toInt();
            settings.setValue("nAnonymizeXCYPTAmount", nAnonymizeXCYPTAmount);
            emit anonymizeXCYPTAmountChanged(nAnonymizeXCYPTAmount);
            break;
